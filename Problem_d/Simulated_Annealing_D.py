...
def add_ship(solution):
	...
	# if all ships are used equally set last value to 0
	if sum(ship[-1] for ship in available_ships) == len(available_ships):
		for ship in available_ships:
			ship[-1] = 0
...
# simulated annealing
def simulated_annealing(solution, runs = 1e2, addshipthreshold = 1e4):
