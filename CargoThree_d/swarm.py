...
def print_ships(ships):
	...
	print "cargo left: weight =", sum(item[KG] for item in ships[-1][CARGO])
	print ",\tvolume = ", sum(item[M3] for item in ships[-1][CARGO])
...
def multiple_motherships(ships, i=10):
	global SHIPS_KG, SHIPS_M3

	ships = [["Cygnus   ", 2000.0*i, 18.9*i, 0.0, [], 2000.0*i, 18.9*i],
			 ["Verne_ATV", 2300.0*i, 13.1*i, 0.0, [], 2300.0*i, 13.1*i],
			 ["Progress ", 2400.0*i,  7.6*i, 0.0, [], 2400.0*i,  7.6*i],
			 ["Kounotori", 5200.0*i, 14.0*i, 0.0, [], 5200.0*i, 14.0*i],
			 ["ThianZhou", 6500.0*i, 15.0*i, 0.0, [], 6500.0*i, 15.0*i],
			 ["Dragon   ", 3400.0*i, 42.0*i, 0.0, [], 3400.0*i, 42.0*i],
			 ["cargo", CARGO_KG, CARGO_M3, 0.0, [], CARGO_KG, CARGO_M3]]
...
def swarm_move(particle, v, gpBest):
	continue


def swarm(ships, cargolist):
	particles = []

	for i in range(30):
		init_ships(ships)
		temp = fill_cargo_random(ships, cargolist)
		hillclimber(temp)
		particles.append(copy.deepcopy(temp))

	pBest, gBest = 0, 0

	for particle in particles: # Calculate Data fitness value
		pScore = score(particle)
        if  pScore > pBest:
            pBest = pScore
        if pScore > gBest:
            gBest = pBest
	...
	gpBest = copy.deepcopy(gpBest)

    for particle in particles:
		pScore = score(particle)
		rand1, rand2 = random.uniform(0.0, 2.0), random.uniform(0.0, 2.0)
		v = rand1*(pBest-pScore)+rand2*(pBest-pScore) # Calculate particle Velocity
		swarm_move(particle, v, gpBest) # Update particle Data
...
def main():
	...
	init_ships(ships)
	solution = swarm(ships, cargolist)
	print_ships(ships)
